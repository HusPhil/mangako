...
const SelectionActionFooter = ({
  ...
  return (
    <Animated.View
      ...
      className="bg-background border-t border-primary/10 px-8 pt-5 z-[60] flex-row justify-center gap-4 shadow-2xl"
    >
      {/* Action: Mark Unread (isRead = false) */}
      <Pressable
        onPress={onMarkUnread}
        className="flex-1 h-14 rounded-xl bg-primary/5 items-center justify-center border border-primary/10 active:bg-primary/10 flex-row"
      >
        <Ionicons name="eye-off-outline" size={22} color="white" />
        <Text className="text-primary font-bold ml-3">Mark Unread</Text>
      </Pressable>

      {/* Action: Mark Read (isRead = true) */}
      <Pressable
        onPress={onMarkRead}
        className="flex-1 h-14 rounded-xl bg-primary items-center justify-center flex-row shadow-lg active:bg-primary/80"
      >
