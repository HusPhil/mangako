...
const MangaHeader = ({
  ...
  const handleAddToList = useCallback(async () => {
    ...
    for (let tabIndex = 0; tabIndex < retrievedMangaList.length; tabIndex++) {
      ...
      const mangaAlreadyAdded = tab.data.some(
        (manga) => manga.link === mangaListItemToAdd.link
      );
      ...
      if (mangaAlreadyAdded) {
        mangaListToSave[tabIndex].data = tab.data.filter(
          (manga) => manga.link !== mangaListItemToAdd.link
        );
      }
    }
  ...
  const renderItem = ({ item, index }) => {
    return (
      <TabListItem
        ...
        iconComponent={
          <MaterialIcons name="check-circle-outline" size={15} color="white" />
        }
        ...
        <View className="h-full w-full justify-center items-center px-3 bg-transparent self-center">
          <View className="w-full bg-secondary rounded-md p-3 max-h-[420px]">
            <Text className="text-white font-pregular text-center pb-2">
              Select where to add this manga
            </Text>
            <HorizontalRule />
            {tabs.length > 0 ? (
              <>
                ...
                />
                <TouchableOpacity
                  className={`border-2 mt-3 border-white rounded-md py-1 px-3 self-center flex-row justify-between items-center`}
        ...
        <View>
          ...
          <View className="flex-row w-full px-2 absolute bottom-5 min-h-[180px] max-h-[185px]">
            <View className="max-h-[180px] max-w-[105px] ml-2">
              ...
              {!isLoading && (
                <TouchableOpacity
                  className={`rounded-md p-1 border-white ${
                    isListed && "bg-accent-100"
                  } border flex-row justify-center items-center`}
                  onPress={handleShowModal}
                >
                  <Text
                    className={`font-pregular text-white text-center mr-1`}
                    style={{
                      fontSize: 11,
                      textShadowColor: "#000",
                      textShadowRadius: 10,
            ...
            <View className="ml-3 mt-1 flex-1 ">
              <Text
                className="text-white font-pmedium text-lg"
                numberOfLines={3}
              >
                {mangaTitle}
              </Text>
              ...
              >
                <Text className="text-white pl-1 font-pregular text-xs pr-4 text-left">
                  {details ? details.desc : "Loading"}
                </Text>
              </ScrollView>
              <Text
                numberOfLines={2}
                className="text-white pl-1 py-2 font-pregular text-xs"
              >
                {details ? `By: ${details.author}` : ""}
              </Text>
            </View>
          </View>
        </View>
