...
const SimpleScraperTest: React.FC = () => {
	const { html, loading, error, logs, webViewProps, scrapeHTML, clearLogs } =
		useSimpleScraper();
	...
	const testUrls = [
		...
		{
			name: 'Your Manga URL',
			url: 'https://mangapark.net/title/74886-en-the-eminence-in-shadow/1784196-ch-1',
		},
	];
		...
		<View style={styles.container}>
			<ScrollView>
				...
				{html && (
					<View style={styles.successSection}>
						...
						<Text style={styles.statsText}>
							Title tags: {(html.match(/<title>/gi) || []).length}
						</Text>
						<Text style={styles.statsText}>
							Body content:{' '}
							{html.includes('<body>') ? 'Yes' : 'No'}
						</Text>
			...
			{webViewProps && (
				<View style={styles.webViewDebug}>
					<Text style={styles.webViewLabel}>
						WebView (for debugging):
					</Text>
