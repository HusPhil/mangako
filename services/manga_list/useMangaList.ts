...
const DEFAULT_MANGA_LIST: MangaList = {
	tabs: [{ id: 'favorites', name: 'Favorites', order: 0, mangaIds: [] }],
	manga: {},
};
...
export function useMangaList() {
	...
	const addMangaToTab = async (tabId: string, manga: Manga) => {
		...
		const updatedTabs = mangaList.tabs.map((tab) =>
			tab.id === tabId
				? {
						...tab,
						mangaIds: [
							...new Set([...tab.mangaIds, manga.mangaId]),
						],
				  }
				: tab
		);
		const updated: MangaList = {
			...
			manga: mangaExists
				? mangaList.manga
				: { ...mangaList.manga, [manga.mangaId]: manga },
		};
		await saveMangaList(updated);
		setMangaList(updated);
	};
	...
	const removeMangaFromTab = async (tabId: string, mangaId: string) => {
		const updatedTabs = mangaList.tabs.map((tab) =>
			tab.id === tabId
				? {
						...tab,
						mangaIds: tab.mangaIds.filter((id) => id !== mangaId),
				  }
				: tab
		);
	...
	const checkIfMangaIsFavorite = async (mangaId: string) => {
		...
		const favoritesTab = currentMangaList.tabs.find(
			(tab) => tab.id === 'favorites'
		);
	...
	const deleteSelectedTabs = async (tabIds: string[]) => {
		...
		const updatedTabs = mangaList.tabs.filter(
	...
	const renameTab = async (tabId: string, newName: string) => {
		...
		const updatedTabs = mangaList.tabs.map((tab) =>
			tab.id === tabId
				? { ...tab, name: newName, id: normalizedName }
				: tab
		);
		const updated: MangaList = {
			tabs: updatedTabs,
	...
	const getMangaListings = async (mangaId: string) => {
		...
		return currentMangaList.tabs.filter((tab) =>
			tab.mangaIds.includes(mangaId)
		);
	};
